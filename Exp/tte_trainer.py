...
from dataset.preprocess_old import Preprocess
...
class Exp:
    def __init__(self, log_path, pretrain_path):
        ...
        self.train_loader = self.dataloader.get_data_loader(self.preprocess_data['train_traj'], is_shuffle=True,sample_ratio=0.17)
        self.eval_loader = self.dataloader.get_data_loader(self.preprocess_data['eval_traj'], is_shuffle=False,sample_ratio=0.17)
        self.test_loader = self.dataloader.get_data_loader(self.preprocess_data['test_traj'], is_shuffle=False,sample_ratio=0.17)
    ...
    #     assert os.path.exists(self.pretrain_path), f'Weights at {self.pretrain_path} not found'
    #
    #     checkpoint = torch.load(self.pretrain_path, map_location='cpu')
    #     print("Checkpoint keys:", checkpoint.keys())
    #
    #     # 初始化 pretraining_model
    #     pretraining_model = GREEN()
    #
    #     # 加载预训练权重
    #     pretraining_model.load_state_dict(checkpoint['model_state_dict'])
    #     pretraining_model.to(self.device)
    #
    #     #封装为 TTE 模型
    ...
    def _build_model(self):
        ...
        n_types = state['type_emb_layer.weight'].shape[0]  # =14

        # 更新 Config
        Config.update({"type_vocab_size": n_types})

        # GREEN 从 Config 读取 type_vocab_size
        pretraining_model = MDTI()

        pretraining_model.load_state_dict(state, strict=True)
        pretraining_model.to(self.device)

        return TTE(pretraining_model)
    ...
    def _evaluation(self, preds, labels , epsilon=1e-3):
        preds = preds.copy()
        preds[preds < 0] = 0
        ...
        mp = preds[mask].ravel()
        ml = labels[mask].ravel()
        mae = mean_absolute_error(ml, mp)
        mape = mean_absolute_percentage_error(ml, mp)
        rmse = mean_squared_error(ml, mp) ** 0.5
        return mae, mape, rmse
    def train(self):
        bad_epoch = 0
        best_epoch = 0
