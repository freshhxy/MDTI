...
import math
import dgl
from model.dgl_gnn import UnsupervisedGAT, UnsupervisedGIN
...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if L > self.pe.size(1):
            # 动态扩容
            new_pe = self._build_pe(L, self.d_model).to(self.pe.device)
            self.pe = new_pe  # buffer 会被更新
